...
Key changes:
1. Removed hard-coded large dictionaries (STCharacters, STPhrases, etc.).
2. Dictionaries are now fetched asynchronously from the OpenCC GitHub repository.
3. The main export is an async function `createConverter` instead of pre-built sync functions.
...
(function(exports) {
    ...
    const promise = fetch(url)
      ...
      .then(text => {
        return text
          ...
          .map(line => line.split('\t').slice(0, 2)); // 每行用 tab 分割，只取前兩項
      });
   ...
   * @param {object} options 轉換選項, 例如 { from: 's', to: 't' }
